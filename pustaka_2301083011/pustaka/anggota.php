...
if ($method === 'POST') {
    ...
    if (isset($data['method']) && $data['method'] === 'login') {
        ...
        $stmt = $koneksi->prepare("SELECT * FROM anggota WHERE email = ? LIMIT 1");
        $stmt->execute([$data['email']]);
        $anggota = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($anggota) {
            // Debug
            error_log("Stored password: " . $anggota['password']);
            error_log("Input password: " . $data['password']);
            
            // Verifikasi password dengan password_verify
            if (password_verify($data['password'], $anggota['password'])) {
                echo json_encode([
                    'status' => 'success',
                    'data' => $anggota
                ]);
